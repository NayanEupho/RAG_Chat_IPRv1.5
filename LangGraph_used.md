# 🧠 LangGraph Deep-Dive: The "Neural" Core of IPR RAG Chat v1.6.1
...
### C. The Retriever (The Platinum Researcher)
...
2.  **Sliding Window Expansion**: If it finds a match, it automatically pulls its **immediate neighbors** (+/- 1 `chunk_index`) to ensure smooth context continuity.
...
4.  **Reranking (FlashRank)**: It takes 20+ candidates and uses a cross-encoder to pick the **Top 7 high-fidelity results**.
5.  **Platinum Enveloping**: Packs each segment into a super-structured Markdown envelope:
    ```markdown
    Source: file.pdf
    Section: A > B > C
    Content: ...
...
## 🏗️ 12. Detailed Node Anatomy: Input, Logic, & Output

This section provides a technical breakdown of how each node interacts with the `AgentState`. 

### 🟢 Node: Planner (Fused Mode)
**The Single-Entry Architect**
- **Inputs**: `messages` (History), `mode` (Auto/RAG/Chat).
- **Internal Logic**: 
    1. **Fast-Path**: Checks keywords and `@mentions` via Regex to bypass LLM if intent is obvious.
    2. **Mega-Prompt**: Calls a high-capacity model with a complex system instruction.
    3. **JSON Extraction**: Validates model output specifically for `intent` and `semantic_queries`.
- **Outputs**: `intent`, `query` (rewritten), `targeted_docs`, `semantic_queries`.
- **Failure Mode**: Defaults to `direct_rag` using the original query if JSON parsing fails.

### 🟡 Node: Router (Modular Mode)
**The Sequential Gatekeeper**
- **Inputs**: `messages`, `mode`.
- **Internal Logic**:
    1. **Heuristic Filter**: Scans for "Hi", "Thanks", or "Help" to force `chat`.
    2. **Vector Probe**: Performs a "Zero-shot" search. If distances match > 0.8, it assumes the query isn't in the knowledge base and routes to `chat`.
- **Outputs**: `intent`, `query_embedding` (passed to Retriever to save costs).

### 🔵 Node: Rewriter (Modular Mode)
**The Cognitive Bridge**
- **Inputs**: `messages`, `query`.
- **Internal Logic**: 
    1. **Context Folding**: Looks at the last 2 human messages to resolve ambiguous pronouns.
    2. **Targeting**: Analyzes if the user is asking about specific files.
- **Outputs**: `semantic_queries` (The structured search roadmap).

### 🟠 Node: Retriever
**The Platinum Researcher**
- **Inputs**: `semantic_queries` (or `query` + `query_embedding` if Planner/Rewriter skipped), `targeted_docs`.
- **Internal Logic**: 
    1. **Segmented Retrieval**: Executes unique searches for every sub-query in the map.
    2. **Sliding Window**: For every high-score hit, it fetches `chunk_index - 1` and `+ 1`.
    3. **Reranking**: Passes raw candidates to **FlashRank** to filter out semantic noise.
- **Outputs**: `documents` (List of super-structured "Platinum Envelopes").

### 🔴 Node: Generator
**The Adaptive Voice**
- **Inputs**: `messages`, `documents`, `intent`.
- **Internal Logic**: 
    1. **Prompt Selection**: Switches between "Base Chat" (no knowledge) and "Augmented RAG" (knowledge base) logic.
    2. **Brevity Control**: Injects `STYLE` instructions based on query complexity.
- **Outputs**: `messages` (Standard AIMessage appended to history).

---

*End of IPR RAG Chat v1.6.1 Deep-Dive.*
