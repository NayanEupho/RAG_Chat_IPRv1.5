...
3. **Safety Guardrails:** Proactively detecting destructive intent and enforcing a human-in-the-loop confirmation step.
4. **Hybrid Execution:** Seamlessly interacting with local processes and remote APIs within the same conversation string.
...
- **Implementation:** It intelligently manages a background `kubectl proxy` process to create a secure, authenticated tunnel to the cluster's API server. This allows clean REST API calls to `127.0.0.1:8001` instead of parsing messy CLI output.
...
- A small model (1B params) is fast but limited.
...
**The "Hot-Swap" Capability** decouples the Agent Logic from the LLM Compute. You can configure:

1. **Fast Agent** runs on `localhost` (using `llama3.2:3b` on the laptop's NPU/GPU).
2. **Smart Agent** runs on a **Remote Server** (e.g., a shared DGX station or HPC cluster) running `llama3.1:70b`.
...
1. **Create Session:** User starts a named session (`devops-agent session start "Debug Auth"`).
...
- Show the full conversation log for any session.
- Clear sessions to free up space.
...
Allowing an AI to control infrastructure is inherently risky. We address this with a **3-Tier Safety Layer**.
...
The system can be started in a strictly **read-only mode**. In this mode, the `DANGEROUS_TOOLS` are completely removed from the prompt context. The AI literally doesn't know that it has the ability to delete things.
...
When a standard CLI command fails, it spits out an error code: `Error: Exit Code 137`. The user has to Google or StackOverflow this. But when our DevOps Agent sees an error, it enters **Analysis Mode**.
...
Agent: "The container db-prod exited with code 1. Let me check the logs."
       [Agent runs docker_logs]

Agent: "The logs show FATAL: password authentication failed for user 'postgres'.
        It seems your environment variable POSTGRES_PASSWORD does not match the 
        config. Please check your .env file."
```
...
> Connecting to Remote Cluster...
> Agent Mode: Smart (System 2)
> Thought: User didn't specify namespace. I'll use the 'find_pod' tool to search globally.
...
2. The Agent polls the active context and sees a session is active.
3. The Complexity Analyzer sees `"why it failed"` and `"remote"` — it selects the **Smart Agent**.
...
- The MCP router directs the JSON-RPC call to `http://localhost:8082`.
- The Remote K8s Server queries the Cloud API and returns a list of 50 pods.
...
The server fetches and aggregates:
...
```
User → CLI → Agent (Parse & Route) → Agent (LLM Reasoning) →
  Router (JSON-RPC) → MCP_Server → K8s API (GET /api/v1/pods/...)
  K8s API → MCP_Server (Pod Details + Events) → Router (Formatted Response) →
  Agent (Tool Result) → Agent (LLM Synthesis) → CLI (Human-Readable Summary) → User
```
...
We plan to add a **Shared Context DB** for team-based debugging.
...
*Technical Report V8 — Nayan Modi, December 2025*
