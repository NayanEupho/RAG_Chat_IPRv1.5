...
- `email` (john.doe@ipr.res.in)
- `uid` (jdoe)
- `employeeID` (12345)

**This implementation assumes `NameID` is the user ID.**
...
We need `python3-saml` for handling the XML crypto and protocol logic.
...
### 2. Configuration (`backend/saml/settings.py`)

This file reads your `.env` variables and prepares the config dict for `python3-saml`.

**Key concept:** We load the IdP certificate from a file (`idp_cert.pem`) or base64 string, and we configure our SP (Service Provider) endpoints to match what we will build in `routes.py`.

### 3. Session Management (`backend/saml/auth.py`)

Since SAML only handles the "login handshake", we need to remember who the user is for subsequent requests. We use a **JWT in a secure HTTP-only cookie**.

- `create_session_token(user_id)`: Mint a new JWT.
- `get_current_user(request)`: FastAPI dependency to protect routes. Checks cookie -> verifies JWT -> returns user_id.

### 4. SAML Endpoints (`backend/saml/routes.py`)

We expose standard SAML endpoints:

- **GET /saml/login**: Redirects user to IdP SSO URL.
- **POST /saml/acs**: "Assertion Consumer Service". IdP posts the XML response here. We parse it, validate signature, extract NameID, set JWT cookie, and redirect to home.
- **GET /saml/metadata**: Returns our SP XML metadata (certs, URLs) so you can register this app with the IdP.

### 5. App Integration (`backend/app.py`)

To make it work:
1. Include the router: `app.include_router(saml_router, prefix="/saml")`
2. **CRITICAL:** Add `CORSMiddleware` with `allow_credentials=True`. Without this, the browser will NOT send the auth cookie in API requests.

### 6. Protecting Data (`backend/state/history.py`)

We modified all database functions to accept a `user_id`:
- `create_session(session_id, user_id)`
- `get_all_sessions(user_id)` -> `SELECT * FROM sessions WHERE user_id = ?`

**Migration Note:** Existing sessions in SQLite typically have no user_id. The code defaults them to "anonymous" or hides them. For a fresh start, deleting the `.sqlite` file is easiest.

---

## Frontend Changes

### 1. Auth Hook (`useAuth.ts`)

This abstract the complexity. It exposes:
- `user`: { id: "john.doe", displayName: "..." }
- `login()`: Redirects browser to `/saml/login`
- `logout()`: Redirects to `/saml/logout`

It automatically checks `/saml/me` on page load to see if we are already logged in.

### 2. Sidebar Updates

- Show "Login" button if guest.
- Show "Logout (john.doe)" if logged in.
- Pass `credentials: 'include'` to `fetch('/sessions')` so the backend knows who is asking for the list.

### 3. Config Fetching

We updated `fetchConfig` (or `checkHealth`) to also respect credentials, though public config usually doesn't need auth.

---

## Environment Variables (.env)

You must configure these for the system to work:

```bash
# SP (Service Provider - This App)
SAML_SP_ENTITY_ID=https://askme.ipr.res.in/saml/metadata
SAML_SP_ACS_URL=https://askme.ipr.res.in/saml/acs
SAML_SP_SETTINGS_URL=https://askme.ipr.res.in/saml/settings

# IdP (Identity Provider - Your SSO Server)
SAML_IDP_ENTITY_ID=http://adfs.ipr.res.in/adfs/services/trust
SAML_IDP_SSO_URL=https://adfs.ipr.res.in/adfs/ls/
SAML_IDP_CERT_FINGERPRINT=... (or use file)

# Security
SAML_SECRET_KEY=... (for signing requests, optional if not enc)
SESSION_SECRET_KEY=... (for signing JWT cookies - CHANGE THIS!)
```
...
- [ ] **Public Access:** Can load the UI (Sidebar should show Login).
- [ ] **Login Flow:** Click Login -> Redirect to IdP -> Authenticate -> Redirect back -> Sidebar shows Name.
- [ ] **Cookie Check:** DevTools -> Application -> Cookies. Should see `session_token` (HttpOnly, Secure/Lax).
- [ ] **Session Isolation:**
   1. User A creates a chat "Project Alpha".
   2. Logout.
   3. Login as User B.
   4. Verify "Project Alpha" is NOT visible.
   5. User B creates "Project Beta".
   6. Logout -> Login User A -> Verify only "Project Alpha" is visible.
