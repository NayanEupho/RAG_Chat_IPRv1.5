...
async def planner_node(state: AgentState):
    ...
    try:
        ...
        if mode == 'auto' and not mentions and q_emb:
            ...
            if len(distances) > 0 and distances[0] > threshold:
                ...
                # Preserve documents from previous RAG turns for source persistence
                return {"intent": "chat", "query": original_query, "targeted_docs": [], "documents": state.get('documents', []), "semantic_queries": [], "query_embedding": q_emb}
    ...
    try:
        ...
        # Preserve documents for chat intents, reset for RAG intents (new retrieval)
        docs_value = state.get('documents', []) if final_intent == "chat" else []
