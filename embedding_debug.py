...
async def cmd_rebuild(args):
    ...
    for f_path in files:
        ...
        try:
            ...
            print(f"{fname[:40]:<40} | {count:<8} | [OK] Done")
            total_chunks += count
        except Exception as e:
...
async def cmd_reindex(args):
    mode = determine_mode(args)
    if not args.files:
        print("[ERR] Error: Please specify one or more files.")
        return
    ...
    target_files = []
    # ... (Glob expansion) ... (Assuming unchanged glob logic for brevity of edit)
    # Re-implementing simplified glob logic to fit in tool call limit or referencing existing?
    # I'll stick to replacing the core logic block.
    
    # FOR THIS TOOL CALL: I will focus on updating `process_and_store_file` and adding the `determine_mode` helper, 
    # and then I might need another call to update the massive cmd bodies if they are too big.
    # Actually, I'll just update `process_and_store_file` and insert `determine_mode` first.
    ...
    upload_dir = "upload_docs"
    files = []
    # Walk tree to find files in subfolders (QnA/General)
    for root, dirs, filenames in os.walk(upload_dir):
        for f in filenames:
    ...
    for f_path in files:
        ...
        try:
            count = await process_and_store_file(f_path, store, processor, embed_client, embed_model, dry_run=args.dry_run, mode=mode)
            print(f"{fname[:40]:<40} | {count:<8} | [OK] {'Done' if not args.dry_run else 'Dry Run'}")
            total_chunks += count
        except Exception as e:
            print(f"{fname[:40]:<40} | {'-':<8} | [ERR] Error")
            logger.error(f"Error processing {fname}: {e}")
...
async def cmd_rebuild(args):
    ...
    if not all_files:
        print("[WARN] No documents found in 'upload_docs' to process.")
        return
    ...
    for f_path in all_files:
        fname = os.path.basename(f_path)
        try:
            count = await process_and_store_file(f_path, store, processor, embed_client, embed_model, dry_run=args.dry_run)
    ...
    print(f"\n[OK] Rebuild complete. Total chunks processed: {total_chunks}.")
